...
export default {
    getByVille: async (req, res) => {
        const { ville } = previsionsParVilleSchema.parse({ ville: req.params.ville });
        const city = CITIES[ville];
        if (!city) {
            throw new NotFoundError(`Ville inconnue. Villes disponibles : ${Object.keys(CITIES).join(', ')}.`);
        }
